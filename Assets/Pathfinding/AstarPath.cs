...
public class AstarPath : MonoBehaviour {
	...
	public AstarClasses.Grid[] grids = new AstarClasses.Grid[1] {new AstarClasses.Grid (20)};
	public AstarClasses.Grid textureGrid = new AstarClasses.Grid (20);
	public AstarClasses.Grid meshGrid = new AstarClasses.Grid (20);
	...
	public void GenerateRotationMatrix (AstarClasses.Grid grid) {
		rotationMatrix = 
	...
	public static Int3 ToLocalTest (Vector3 pos,bool forceNodesUnder) {
		...
		for (int y=0;y<active.grids.Length;y++) {
            AstarClasses.Grid grid = active.grids[y];
	...
	public void SetNodes (bool walkable, Vector3 point, int gridIndex,bool allGrids) {
		...
		if (allGrids) {
			...
			if (p != new Int3 (-1,-1,-1)) {
			AstarClasses.Grid grid = grids[p.y];
				Node node = GetNode(p);
				node.walkable = walkable;
				RecalcNeighbours (node);
	...
	AstarClasses.Grid grid = grids[gridIndex];
	...
	public void FullPhysicsCheck (Node node, AstarClasses.Grid grid) {
	...
	public void FullPhysicsCheck (Node node, AstarClasses.Grid grid, LayerMask extraMask, bool doYTest) {
	...
	AstarClasses.Grid grid = grids[node.pos.y];
	...
	AstarClasses.Grid grid = grids[pos.y];
		for (int x=pos.x;x<pos.x+w;x++) {
			for (int z=pos.z;z<pos.z+w;z++) {
	...
	AstarClasses.Grid[] allGrids = new AstarClasses.Grid[nodes.Length];
		for (int i=0;i<allGrids.Length;i++) {
			allGrids[i] = new AstarClasses.Grid ();
		...
		for (int i=0;i<allGrids.Length;i++) {
		AstarClasses.Grid grid = allGrids[i];
		...
		for (int y=0;y<allGrids.Length;y++) {
		AstarClasses.Grid grid = allGrids[y];
	...
	AstarClasses.Grid grid = new AstarClasses.Grid (20);
		grids = new AstarClasses.Grid[1] {grid};
		grid.width = points.Length;
		grid.depth = 1;
	...
	public void LoadAstarData () {
		if (astarData != null) {
			...
			grids = new AstarClasses.Grid[1] {astarData.grid};
			meshGrid = grids[0];
