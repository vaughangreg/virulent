...
/*public class UploadWrapper
{
	...
	public List<System.Object> dataObjs;
	...
	public void Add(ADAGEData newData)
	{
		...
		bool typeFound = types.Contains(newData.GetType().ToString());
		if(newData.unique && typeFound)
		{
			newData = ADAGEData.Copy(newData);
		}
	
		dataObjs.Add(JsonMapper.ToJson(newData));

	

		if(data.Count == 0 || data[data.Count - 1] != newData) //if it's the first element or the element isn't the same as the last element addeds
		{
			data.Add(newData);	

			if(!typeFound)
				types.Add(newData.GetType().ToString());
		}
	}
...
public class ADAGE : MonoBehaviour
{
	...
	public const string k_OnError = "ADAGE.OnError(string)";
	...
	private Dictionary<int, Dictionary<string, ADAGEData>> userCache;
...
//All player login and registration functions will return the local player id that should be used to reference this player for data logging. 
	...
	public static int AddUser(ADAGEUser newUser = null)
	{
		...
		if(instance.userCache == null)
			instance.userCache = new Dictionary<int, Dictionary<string, ADAGEData>>();

		/*if(users.Count >= instance.maxUsers)
		{
			Messenger<string>.Broadcast(ADAGE.k_OnError, "Unable to add ADAGE user. Maximum reached");
			return -1;
		}*/

		instance.userCache.Add(users.Count, new Dictionary<string, ADAGEData>());
	...
	public static int RegisterPlayer(string playerName, string email, string password, string passwordConfirm)
	{
		if(IsNull)
			return -1;
	...
	private void StartSession(int localId)
	{
		...
		ADAGEStartSession log_session = new ADAGEStartSession();
		LogData<ADAGEStartSession>(log_session, localId);
	}
	...
	/*public void OnGUI()
	{
		...
		float infoScreenWidth = Screen.width / 4f;
		float infoScreenHeight = Screen.height / 3f;
		Rect infoScreen = new Rect(0,0, infoScreenWidth, infoScreenHeight);

		float labelWidth = infoScreenWidth * 0.3f;
		GUI.BeginGroup(infoScreen,"","box");
		{
			GUILayout.Label("ADAGE Information", titleStyle);
			GUILayout.BeginVertical();
			{
				GUILayout.BeginHorizontal();
				{
					GUILayout.Label("Group: ", labelStyle, GUILayout.Width(labelWidth));
					GUILayout.Label("MyGroup", infoStyle);
				}
				GUILayout.EndHorizontal();

				GUILayout.BeginHorizontal();
				{
					GUILayout.Label("Session: ", labelStyle, GUILayout.Width(labelWidth));
					GUILayout.Label(currentSession, infoStyle);
				}
				GUILayout.EndHorizontal();

				GUILayout.BeginHorizontal();
				{
					GUILayout.Label("Game: ", labelStyle, GUILayout.Width(labelWidth));
					GUILayout.Label("MyGame", infoStyle);
				}
				GUILayout.EndHorizontal();
			}
...
//		Debug.Log (path);
	...
	private void OnADAGEWebJobComplete(Job job)
	{
		...
		try
		{
			...
			if(ReflectionUtils.CompareType(errorType, typeof(ADAGEErrorResponse)))
			{
				...
				Messenger<string>.Broadcast(k_OnError, statusMessage);
				...
				if(ReflectionUtils.CompareType(errorType, typeof(ADAGEHostError)) || statusMessage.Trim() == "Could not find application.")
				{
					...
					else //if we aren't, we need to display an error
					{
						...
						{
							Messenger<string>.Broadcast(k_OnError, "Unable to resolve host");
						}
					}
				}
		...
		catch(Exception e)
		{
			if (ADAGEMenu.instance != null)
			{
				ADAGEMenu.ShowError(e.ToString());
			}
	...
	{
		ADAGEClientJob connection = (job as ADAGEClientJob);
	...
	{
		if(typeof(ADAGEContextStart).IsAssignableFrom(data.GetType()))
		{
			StartContext(data as ADAGEContextStart, localId);
		}
		else if(typeof(ADAGEContextEnd).IsAssignableFrom(data.GetType()))
		{
			EndContext(data as ADAGEContextEnd, localId);			
		}		
		
		AddData<T>(data, localId);
	}
	
	private void StartContext(ADAGEContextStart data, int localId)
	{	

		users[localId].vContext.Add(data.name);
	}

	private void EndContext(ADAGEContextEnd data, int localId)
	{		
		users[localId].vContext.Remove(data.name);
	}
