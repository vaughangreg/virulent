		...
		GUILayout.BeginVertical();
		{
			...
			GUILayout.BeginHorizontal();
			{
				GUILayout.BeginVertical("box", GUILayout.MaxWidth(position.width / 2f - 2.5f), GUILayout.MinHeight(position.height - 70f));
				{
					...
					{
						if(!refreshing && curTarget.versionInfo != null && curTarget.versionInfo.eventTypes != null && curTarget.versionInfo.eventTypes.Count > 0)
						{
							List<string> events = new List<string>(curTarget.versionInfo.eventTypes.Keys);
							for(int i = 0; i < events.Count; i++)
							{
								string eventName = events[i];
								ADAGEEventInfo eventInfo = curTarget.versionInfo.eventTypes[eventName];
						...
						else if(curTarget.versionInfo.eventTypes == null || curTarget.versionInfo.eventTypes.Count == 0)
						{
